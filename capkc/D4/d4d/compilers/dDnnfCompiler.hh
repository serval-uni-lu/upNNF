...
inline bool isVarInVec(Var const v, vec<Lit> const& lits) {
...
private:
    ...
    */
    std::shared_ptr<DAG<T> > compile_(vec<Var> &setOfVar, vec<Var> &priorityVar, Lit dec, onTheBranch &onB,
                     bool &fromCache, vec<int> &idxReason, int const max_depth)
    {
        fromCache = false;
        ...
        if(!nbComponent || max_depth <= 0)
        {
            comeFromCache.push_back(false);
            for(std::size_t i = 0; i < reallyPresent.size(); i++) {
                if(! isVarInVec(reallyPresent[i], onB.units)) {
                    onB.free.push(reallyPresent[i]);
                }
            }
        ...
        else
        {
            ...
            for(int cp = 0 ; cp<nbComponent ; cp++)
            {
                ...
                else
                {
                    ...
                    ret = compileDecisionNode(connected, currPriority, max_depth - 1);
    ...
    */
    std::shared_ptr<DAG<T> > compileDecisionNode(vec<Var> &connected, vec<Var> &priorityVar, int const max_depth)
    {
        ...
        std::shared_ptr<DAG<T> > pos = compile_(connected, priorityVar, l, bPos, fromCachePos, idxReason, max_depth);
        (s.assumptions).pop();
        (s.cancelUntil)((s.assumptions).size());

        // compile the formula where l is assigned to true
        (s.assumptions).push(~l);
        std::shared_ptr<DAG<T> > neg = compile_(connected, priorityVar, ~l, bNeg, fromCacheNeg, idxReason, max_depth);
...
public:
    ...
    */
    ...
    {
        optMaxDepth = optList.maximumDepth;
    ...
    std::unique_ptr<rootNode<T> > compile()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                d = compile_(setOfVar, priorityVar, lit_Undef, bData, fromCache, idxReason, optMaxDepth);
            }
