...
With cubes, the upper bound (built with the `divkc` theorems) has `159144` solutions (about `11` times less by using only `10` cubes) and the number of tries by the sampler plummeted from `108` to `15` with the usage of cubes.
