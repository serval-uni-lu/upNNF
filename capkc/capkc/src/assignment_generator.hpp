...
/**
  * \returns The number of clauses that don't share at least one literal with the given cube
  * \brief If it returns 0, then the cnf is satisfied by the cube
  */
...
struct Cube {
    /**
      * \brief The literals of the cube
      */
    LitSet lits;

    /**
      * \brief The number of solutions under this edge of the d-DNNF according to the DivKC theorems.
      */
    mpz_int mc;
};
...
/**
  * \brief Reads the cube list generated by the dnf executable
  */
...
struct Cube {
    /**
      * \brief The literals in this cube
      */
    LitSet m;

    /**
      * \brief The number of clauses in the CNF formula that don't share at least one literal with this cube
      */
    std::size_t nb_unsat;
};
...
public:
    ...
    /**
      * \details Given that this generator generates cubes from the upper bound G_U = empty set,
      * the model count of the upper bound returned is 2^(number of variables).
      */ 
    ...
    /**
      * \returns A randomly generated cube where each variable has a 0.5 chance of being positive
      */
    template<typename PRNG>
...
public:
    /**
      * \details
      * Constructs the assignment generator.
      * This is intended for the case where cnf is the base formula and annf is
      * an annotated NNF (with model counts) that is either an upper bound of the cnf
      * or the d-DNNF form of the cnf
      */
    ...
    /**
      * \returns The number of models of the given annf
      */
    inline mpz_int get_mc() const { return annf.mc(NNF::ROOT); }

    /**
      * \returns A model uniformly sampled from the d-DNNF given at construction.
      * nb_unknown(cnf, res) is computed and stored in Cube.nb_unsat
      */
    template<typename PRNG>
...
public:
    /**
      ...
      * an annotated NNF (with model counts) that is either an upper bound of the cnf
      * or the d-DNNF form of the cnf.
      * Additionally, this generator uses the cubes generated by the dnf executable
      * and the DivKC theorems to further tighten the upper bound.
      */
    ...
    /**
      * \returns The model count of the upper bound
      */
    inline mpz_int get_mc() const { return mc; }

    /**
      * \returns A model uniformly sampled from the d-DNNF tightened with the cubes given at construction.
      * nb_unknown(cnf, res) is computed and stored in Cube.nb_unsat
      */
    template<typename PRNG>
    Cube operator()(PRNG & gen) {
...
/**
  * \details
  * Uses Monte carlo and rejection sampling to produce samples for the CNF formula by using
  * the given generator.
  */
template<typename G>
class MonteCarlo {
...
public:
    /**
      * \details
      * Possible values for gen are RandomAssignmentGenerator, RandomNNFAssignmentGenerator and RandomCubeAssignmentGenerator.
      * The generator gen is expected to have a similar interface:
      * ```cpp
      * Cube operator()(PRNG & gen)
      * inline mpz_int get_mc()
      * inline CNF const& get_cnf()
      * ```
      */
    ...
    /**
      ...
      * \param prng the pseudo random number generator to use. It is expected to have jump functions.
      */
    template<typename PRNG>
...
/**
  * \brief A sampler based on the splitting method. Works poorly at best
  */
template<typename G>
...
/**
  * \brief A sampler based on the metropolist hastings algorithm and random walk.
  * \details
  * There is descent, burnin and sampling phases which can have individual settings.
  * The algorithm is hybrid, meaning that sometimes a pure random walk will be performed
  * and sometimes the metropolis hastings move will be done (nore none at all if MH decides so).
  * The probabilities depend on the settings.
  */
template<typename G>
