...
public:
    ...
    template<typename PRNG>
    void run(std::size_t const N, PRNG & prng) {
        std::cout << "c Sampling\n";
        ...
        while(nb_done < N) {
            ...
            while(true) {
                ...
                if(population[0].nb_unsat == 0) {
                    for(std::size_t j = 0; j < population.size() && nb_done < N && population[j].nb_unsat == 0; j++) {
                        if(print) {
...
public:
    ...
    void run(std::size_t const N, PRNG & prng) {
        ...
        std::cout << "c nb samples generated: " << nb_done << "\n";
